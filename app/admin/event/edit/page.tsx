...
const EventRegister = () => {
  ...
  useEffect(() => {
    // Set the form values when eventData is updated
    if (!initialLoading && eventData && !formInitialized) {
      form.setValues({
        ...
        organizingOrganization: eventData.organizingOrganization || '',
        targetAudience: eventData.targetAudience || '',
        activities: eventData.activityDetails || [], // name, description
        volunteering: eventData.volunteeringDetails || {}, // isVolunteer: Boolean, contact: String
        donations: eventData.donationDetails || {}, // isDonations: Boolean, contact: String
        logo: eventData.logo || '',
        contactInformation: eventData.contactInformation || {}, // phone, website, email
        socialMediaLinks: eventData.socialMedia || {}, // twitter, linkedIn
        registrationLink: eventData.registrationDetails || {}, // isregistration: Boolean, link: String
      });
      setFormInitialized(true);
    }
  }, [eventData, initialLoading, formInitialized]);
  const getData = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setContactInfomation(response.event.contactInformation);
        setActivityDetails(response.event.activities);
        setVolunteeringDetails(response.event.Volunteering);
        setDonationDetails(response.event.donations);
        setSocialMedia(response.event.socialMediaLinks);
        setRegistrationDetails(response.event.registrationLink);
        ...
        <Stepper active={active} breakpoint="sm">
          <Stepper.Step mt={20} label="First step" description="Event details">
            ...
            <MultiSelect
              ...
              onChange={(value) =>
                form.setFieldValue('eventType', JSON.stringify(value))
              }
            />
            <TextInput
              mt={30}
