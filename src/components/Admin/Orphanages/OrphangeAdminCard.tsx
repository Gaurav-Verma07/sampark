...
interface DeleteCardType {
  orphanage: OrphanageCardType;
  setDeleteId: (_id: string) => void;
}

export default function OrphanageAdminCard(props: DeleteCardType) {
  const { _id, name, address, description, image }: OrphanageCardType =
    props.orphanage;
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch('/api/admin/orphanage/delete', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ id: _id }),
             ...
             if(res.status===200){
               toast.success('Orphanage is successfully deleted', {
        position: 'top-right',
        autoClose: 5000,
